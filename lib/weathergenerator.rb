...
class WeatherGenerator
  ...
  def initialize (baselinedata, precipitation_occurance_chart, month, num_days, dieroller)
    ...
    while num_days > 0
      if (num_days > 28)
        28.times { @days.push make_weather(baselinedata.month(month), dieroller, precipitation_occurance_chart) }
        num_days -= 28
        month += 1
        ...
        num_days.times { @days.push make_weather(baselinedata.month(month), dieroller, precipitation_occurance_chart) }
        num_days = 0
      end
    end
  end
